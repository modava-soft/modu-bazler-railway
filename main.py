# -*- coding: utf-8 -*-
# Modu Bazler – Watchlist Pro v5 (main.py با منوی چندصفحه‌ای و نمودار+آلارم تک‌نماد)
...
def menu_page_1():
    ...
    kb.add(types.InlineKeyboardButton("نمودار+آلارم تک‌نماد", callback_data="single_combo"))
...
def register_handlers(bot, bot_name):
    ...
    def callback_router(c):
        ...
        if data == "help":
            txt = "راهنما:\n"
            txt += "صفحه ۱: نمودار+آلارم تک‌نماد، لیست ارزها، واچ‌لیست‌ها، شروع چرخه‌ها، اجرای فوری ۱h\n"
        ...
        if data.startswith("tf_") and ("_single" in data or "_chart" in data):
            state = STATE[bot_name].get(chat_id)
            if isinstance(state, tuple) and state[0] == "single_combo":
                symbol = state[1]
            elif isinstance(state, tuple) and state[0] == "single_symbol":
                symbol = state[1]
            else:
                bot.answer_callback_query(c.id)
                return
